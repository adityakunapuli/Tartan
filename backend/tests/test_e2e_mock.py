...
def test_full_pipeline_execution(mock_llm, mock_get_client, mock_plaid_env, db_session):
    ...
    mock_sync.__getitem__.side_effect = (
        lambda k: False if k == "has_more" else getattr(mock_sync, k)
    )
    ...
    def inv_tx_getitem(name):
        ...
        if name == "securities":
            return []
        if name == "total_investment_transactions":
            return 0
        return MagicMock()
    ...
    mock_liab_resp.__getitem__.side_effect = (
        lambda k: {} if k == "liabilities" else MagicMock()
    )
