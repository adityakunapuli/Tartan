"""FastAPI routes for Plaid Link token exchange."""
import time
import plaid
...
async def create_link_token() -> dict:
    """Creates a Plaid Link token for initializing Link flow."""
    try:
...
async def set_access_token(request: TokenExchangeRequest) -> dict:
    """Exchanges a public token for an access token and returns it."""
    try:
