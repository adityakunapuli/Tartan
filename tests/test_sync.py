...
from analysis.services.sync import sync_transactions, sync_accounts, sync_holdings
from analysis.db.models import Transaction, PlaidItem, Account, InvestmentHolding
...
class MockSyncResponse:
    def __init__(self, added, modified, removed, next_cursor, has_more, item_id="item_123"):
        self.added = added
...
class MockAccount:
    def __init__(self, id, current, available=None):
        ...
        self.balances = MagicMock(current=current, available=available, iso_currency_code="USD", limit=None)
        self.apy = 0.0
...
def test_sync_accounts(db_session, mock_plaid_client):
    ...
    mock_plaid_client.accounts_get.return_value = MockAccountResponse([acc], inst_id="ins_test")
    ...
    # Verify DB
    saved_acc = db_session.execute(select(Account).where(Account.account_id == "acc_1")).scalar_one()
...
def test_sync_transactions_incremental(db_session, mock_plaid_client):
    ...
    resp1 = MockSyncResponse(added=[tx1], modified=[], removed=[], next_cursor="cursor_2", has_more=False)
    ...
    # Verify DB
    tx = db_session.execute(select(Transaction).where(Transaction.transaction_id == "tx_1")).scalar_one()
    assert tx.name == "Walmart"
    assert tx.amount == 50.0
    
    # Verify Cursor Update
    item = db_session.execute(select(PlaidItem).where(PlaidItem.access_token == token)).scalar_one()
...
def test_sync_transactions_removals(db_session, mock_plaid_client):
    ...
    db_session.add(Transaction(transaction_id="tx_del", account_id="acc_1", name="Old", amount=10.0))
    db_session.commit()
    
    # Mock Response: 1 Removed
    removed_item = {"transaction_id": "tx_del"}
    resp = MockSyncResponse(added=[], modified=[], removed=[removed_item], next_cursor="c2", has_more=False)
    ...
    # Verify Removal
    tx = db_session.execute(select(Transaction).where(Transaction.transaction_id == "tx_del")).scalar_one_or_none()
    assert tx is None
...
def test_sync_holdings_cleanup(db_session, mock_plaid_client):
    ...
    # Existing Holding
    h1 = InvestmentHolding(date_captured=today, account_id="acc_inv", security_id="sec_1", quantity=10, institution_value=100)
    db_session.add(h1)
    db_session.commit()
