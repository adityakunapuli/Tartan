...
def print_spending_by_category() -> None:
    ...
    category_sum = spending.groupby('enriched_category')['amount'].sum().sort_values(ascending=False)
...
def print_historical_performance() -> None:
    ...
    sells = df_it[df_it['type'] == 'sell']['amount'].sum() # Assuming sell exists, otherwise 0
