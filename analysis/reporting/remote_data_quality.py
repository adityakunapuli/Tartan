...
def analyze_transaction_data() -> None:
    ...
    print(f"Transactions with Merchant Name: {len(df) - missing_merchant} ({(len(df) - missing_merchant)/len(df):.1%})")
    print(f"Transactions MISSING Merchant Name: {missing_merchant} ({(missing_merchant)/len(df):.1%})")
    ...
    if missing_merchant > 0:
        ...
        missing_sample = df[df['merchant_name'].isnull()].sample(min(10, missing_merchant))
    ...
    if not df[df['merchant_name'].notnull()].empty:
        ...
        if not inconsistent.empty:
            print(f"Found {len(inconsistent)} merchants with multiple Plaid categories:")
            ...
            print(df[df['merchant_name'] == example_merchant][['date', 'name', 'category']].to_string())
        else:
            print("Plaid categories are consistent per merchant (for the data we have).")
