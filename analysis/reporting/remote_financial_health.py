...
def analyze_financial_health() -> None:
    ...
    if deployable_cash > 0:
        print(f"Γ£à SURPLUS DETECTED: You have ${deployable_cash:,.2f} in excess cash.")
        
        if current_debt > 0:
            pay_amount = min(deployable_cash, current_debt)
            print(f"≡ƒÜ¿ STRATEGY ALERT: Pay off ${pay_amount:,.2f} of debt IMMEDIATELY.")
